...
cdnDict = {}
with open(r'cdn-domain.conf', 'rt', encoding='utf-8') as f:
...
def query_A(subdomain):
    ...
    ips = []
    try:
        dns_A_ips = [j for i in dns.resolver.resolve(subdomain, 'A').response.answer for j in i.items]
        ips = []
    ...
    while not subdomains_queue.empty():
        ...
        if not cnameRet:
            ...
            if not ipASNSRet:
                ...
                # print('{}: {}'.format(subdomain, ipASNSRet))
                # notCDNSubdomains.append(subdomain)
                CDNSubdomainsDict[subdomain] = ipASNSRet
        else:
            # print('{}: {}'.format(subdomain, cnameRet))
            # notCDNSubdomains.append(subdomain)
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        for domain in notCDNSubdomains:
            print(domain)
    else:
        print(f"Usage: \n\tpython3 {sys.argv[0]} domainList.txt")
